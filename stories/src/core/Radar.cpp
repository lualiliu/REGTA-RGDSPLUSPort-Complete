...
bool CRadar::m_bDrawingBottomScreen;
#endif
...
#ifdef ENABLE_3DS_BOTTOM_RADAR
static int
ClipBottomRadarEdge(const CVector2D *input, int count, CVector2D *output,
	int axis, float boundary, bool keepGreater)
{
	if(count == 0)
		return 0;
	int outputCount = 0;
	CVector2D previous = input[count - 1];
	...
	for(int i = 0; i < count; i++) {
		CVector2D current = input[i];
		float currentValue = axis == 0 ? current.x : current.y;
		bool currentInside = keepGreater ? currentValue >= boundary : currentValue <= boundary;
		if(currentInside != previousInside) {
			float denominator = currentValue - previousValue;
			float t = denominator == 0.0f ? 0.0f : (boundary - previousValue) / denominator;
...
static int
ClipBottomRadarPoly(CVector2D *output, const CVector2D *input)
{
	CVector2D a[12], b[12];
	for(int i = 0; i < 4; i++)
		a[i] = input[i];
	int count = 4;
	count = ClipBottomRadarEdge(a, count, b, 0, -1.0f, true);
	count = ClipBottomRadarEdge(b, count, a, 0, 5.0f / 3.0f, false);
	count = ClipBottomRadarEdge(a, count, b, 1, -1.0f, true);
	count = ClipBottomRadarEdge(b, count, a, 1, 1.0f, false);
	for(int i = 0; i < count; i++)
		output[i] = a[i];
	return count;
}
...
int CRadar::ClipRadarPoly(CVector2D *poly, const CVector2D *rect)
{
	...
	if(m_bDrawingBottomScreen)
		return ClipBottomRadarPoly(poly, rect);
#endif
	CVector2D corners[4] = {
...
{
	CVector planes[4] = {
		/* The lower radar is centred at x=120 and spans the whole 320-pixel
		 * display.  The -x+z plane is its right edge, so z is 5/3. */
		CVector(-1.0f, 0.0f,
#ifdef ENABLE_3DS_BOTTOM_RADAR
			m_bDrawingBottomScreen ? 5.0f / 3.0f : 1.0f),
#else
			1.0f),
#endif
...
#ifdef ENABLE_3DS_BOTTOM_RADAR
		if(m_bDrawingBottomScreen)
			m_radarRange = Max(m_radarRange, 180.0f);
#endif
...
void CRadar::DrawRadarMap()
{
	...
	/* Keep the exact reVC lower-radar path.  Its 180-unit range is covered by
	 * the ordinary 3x3 resident tile set; replacing this with a second custom
	 * streaming window left the rail-side portion without resident textures. */
...
#ifdef ENABLE_3DS_BOTTOM_RADAR
	if(m_bDrawingBottomScreen) {
		// reVC lower-screen contract: off-radar icons travel around the inset
		// rectangular edge of the complete 320x240 display, not a unit circle.
		const float left = -11.0f / 12.0f;
		const float right = 19.0f / 12.0f;
		const float vertical = 11.0f / 12.0f;
		if(dist > 1.0f) {
			float edgeScale = 1000000.0f;
			if(point.x < 0.0f)
				edgeScale = Min(edgeScale, left / point.x);
			else if(point.x > 0.0f)
				edgeScale = Min(edgeScale, right / point.x);
			if(point.y < 0.0f)
				edgeScale = Min(edgeScale, -vertical / point.y);
...
void CRadar::TransformRadarPointToScreenSpace(CVector2D &out, const CVector2D &in)
{
	...
	if (FrontEndMenuManager.m_bMenuMapActive) {
		 ...
		 * instead of assuming that world (0, 0) lies at the texture centre. */
		const float worldX = in.x * m_radarRange;
		const float worldY = in.y * m_radarRange;
		const float sourceX = LCS_MENU_MAP_X_OFFSET +
			worldX * LCS_MENU_MAP_XX + worldY * LCS_MENU_MAP_XY;
		const float sourceY = LCS_MENU_MAP_Y_OFFSET +
			worldX * LCS_MENU_MAP_YX + worldY * LCS_MENU_MAP_YY;
		const float scale = FrontEndMenuManager.m_fMapSize * 2.0f / 220.0f;
		out.x = FrontEndMenuManager.m_fMapCenterX -
			FrontEndMenuManager.m_fMapSize * (320.0f / 220.0f) + sourceX * scale;
		out.y = FrontEndMenuManager.m_fMapCenterY -
			FrontEndMenuManager.m_fMapSize + sourceY * scale;
#else
...
CRadar::DrawLegend(int32 x, int32 y, int32 sprite)
{
	...
	switch ( sprite ) {
		...
		break;
		case RADAR_SPRITE_EIGHTBALL:
			text = TheText.Get("LG_38");
		break;
		case RADAR_SPRITE_BURNER:
		...
		case RADAR_SPRITE_TOSHIKO:
			text = TheText.Get("LG_46");
		break;
		case RADAR_SPRITE_VINCENZO:
			text = TheText.Get("LG_47");
		break;
		case RADAR_SPRITE_BOMB:
			text = TheText.Get("LG_48");
		break;
		case RADAR_SPRITE_CLOTHES:
			text = TheText.Get("LG_49");
		break;
		case RADAR_SPRITE_PHIL_CASSIDY:
			text = TheText.Get("LG_50");
		break;
		case RADAR_SPRITE_POWERUP:
			text = TheText.Get("LG_53");
		break;
