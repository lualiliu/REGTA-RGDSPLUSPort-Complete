...
#ifdef _3DS
static bool ActivateLCSCheatAlias(const char *cheat);
...
{
	if(touch.px >= 23 && touch.px < 148 && touch.py >= 26 && touch.py < 105)
...
static void
Open3DSCheatKeyboard(void)
{
	SwkbdState keyboard;
	char cheat[30] = { 0 };
	swkbdInit(&keyboard, SWKBD_TYPE_QWERTY, 2, 29);
	swkbdSetHintText(&keyboard, "Enter cheat code");
	swkbdSetValidation(&keyboard, SWKBD_NOTEMPTY_NOTBLANK, 0, 0);
	if(swkbdInputText(&keyboard, cheat, sizeof(cheat)) != SWKBD_BUTTON_CONFIRM)
		return;
	for(char *c = cheat; *c; c++)
		if(*c >= 'a' && *c <= 'z')
			*c -= 'a' - 'A';
	ActivateLCSCheatAlias(cheat);
}
#endif
...
void BigHeadsCheat()
{
	...
	CHud::SetHelpMessage(TheText.Get(CPed::bBigHeadsCheat ? "CHEAT1" : "CHEATOF"), true);
}

void MaleFollowersCheat()
{
	CPed::bMaleMagnetCheat = !CPed::bMaleMagnetCheat;
	CHud::SetHelpMessage(TheText.Get(CPed::bMaleMagnetCheat ? "CHEAT1" : "CHEATOF"), true);
}

void BikeTiresCheat()
{
	CVehicle::bCheat9 = !CVehicle::bCheat9;
	CHud::SetHelpMessage(TheText.Get(CVehicle::bCheat9 ? "CHEAT1" : "CHEATOF"), true);
}
...
void UpsideDownCheat(bool enabled)
{
	gLCSUpsideDownCheat = enabled;
	CHud::SetHelpMessage(TheText.Get(enabled ? "CHEAT1" : "CHEATOF"), true);
}
...
#ifdef _3DS
static void
ShowLCSTeleportMessage(const char *message)
{
	static wchar unicodeMessage[64];
	AsciiToUnicode(message, unicodeMessage);
	CHud::SetHelpMessage(unicodeMessage, true);
}

static bool
GetLCSBlipPosition(const sRadarTrace &trace, CVector &position)
{
	CEntity *entity = nil;
	switch(trace.m_eBlipType) {
	case BLIP_COORD:
	case BLIP_CONTACT_POINT:
		position = trace.m_vecPos;
		return true;
	case BLIP_CAR:
		entity = CPools::GetVehiclePool()->GetAt(trace.m_nEntityHandle);
		break;
	case BLIP_CHAR:
		entity = CPools::GetPedPool()->GetAt(trace.m_nEntityHandle);
		break;
	case BLIP_OBJECT:
		entity = CPools::GetObjectPool()->GetAt(trace.m_nEntityHandle);
		break;
	default:
		break;
	}
...
static bool
IsLCSMainMissionSprite(uint16 sprite)
{
	switch(sprite) {
	case RADAR_SPRITE_BURNER:   // Ned Burner's church-confessional missions
	case RADAR_SPRITE_CIPRIANI:
	case RADAR_SPRITE_DONALD:
	case RADAR_SPRITE_JOSEPH:
	case RADAR_SPRITE_LEONE:
	case RADAR_SPRITE_MARIA:
	case RADAR_SPRITE_MCRAY:
	case RADAR_SPRITE_TOSHIKO:
	case RADAR_SPRITE_VINCENZO:
		return true;
	default:
		return false;
	}
...
/* Mission and safehouse blips sit on script trigger points. Use them only as
 * anchors and land on a nearby path node so streaming and the destination
 * script cannot both fire on the first post-keyboard frame. */
static bool
FindLCSTeleportLanding(const CVector &anchor, CVector &landing)
{
	const float desiredDistance = 28.0f;
	const float minimumDistance = 18.0f;
	const float maximumDistance = 65.0f;
	eLevelName anchorLevel = CTheZones::GetLevelFromPosition(&anchor);
	float bestScore = 1000000.0f;
	bool found = false;
	for(int32 i = 0; i < ThePaths.m_numCarPathNodes; i++) {
		const CPathNode &node = ThePaths.m_pathNodes[i];
		if(node.bDisabled || node.bBetweenLevels || node.bWaterPath)
			continue;
		CVector candidate = ThePaths.m_pathNodes[i].GetPosition();
		if(CTheZones::GetLevelFromPosition(&candidate) != anchorLevel)
			continue;
		float dx = candidate.x - anchor.x;
		float dy = candidate.y - anchor.y;
		float distance = Sqrt(dx * dx + dy * dy);
		if(distance < minimumDistance || distance > maximumDistance || Abs(candidate.z - anchor.z) > 10.0f)
			continue;
		float score = Abs(distance - desiredDistance) + 0.2f * Abs(candidate.z - anchor.z);
		if(score < bestScore) {
			landing = candidate;
			bestScore = score;
			found = true;
...
static bool
TeleportLCSToBlip(const CVector &anchor)
{
	CPlayerPed *player = FindPlayerPed();
	if(player == nil)
		return false;
	CVehicle *vehicle = FindPlayerVehicle();
	CVector originalPosition = FindPlayerCoors();
	eLevelName originalLevel = CGame::currLevel;
	int32 originalArea = CGame::currArea;
	CVector landing;
	if(!FindLCSTeleportLanding(anchor, landing)) {
		ShowLCSTeleportMessage("NO SAFE OUTDOOR LANDING FOUND");
		return false;
	}

	CGame::currArea = AREA_MAIN_MAP;
	CStreaming::RemoveBuildingsNotInArea(AREA_MAIN_MAP);
	CGame::currLevel = CTheZones::GetLevelFromPosition(&landing);
	CStreaming::LoadScene(landing);
	CStreaming::LoadSceneCollision(landing);

	bool foundGround = false;
	float expectedRoadZ = landing.z;
	float ground = CWorld::FindGroundZFor3DCoord(landing.x, landing.y, expectedRoadZ + 3.0f, &foundGround);
	if(!foundGround || Abs(ground - expectedRoadZ) > 6.0f ||
	   !CWorld::GetIsLineOfSightClear(CVector(landing.x, landing.y, ground + 0.5f),
	                                  CVector(landing.x, landing.y, ground + 6.0f),
	                                  true, false, false, true, true, false)) {
		CGame::currArea = originalArea;
		CGame::currLevel = originalLevel;
		CStreaming::RemoveBuildingsNotInArea(originalArea);
		CStreaming::LoadScene(originalPosition);
		CStreaming::LoadSceneCollision(originalPosition);
		ShowLCSTeleportMessage("NO SAFE OUTDOOR LANDING FOUND");
		return false;
	}
	landing.z = ground;

	CEntity *entity = vehicle ? (CEntity*)vehicle : (CEntity*)player;
	landing.z += entity->GetDistanceFromCentreOfMassToBaseOfModel() + 0.15f;
	entity->Teleport(landing);
	CTheScripts::ClearSpaceForMissionEntity(landing, entity);
	if(vehicle) {
		vehicle->SetMoveSpeed(0.0f, 0.0f, 0.0f);
		vehicle->SetTurnSpeed(0.0f, 0.0f, 0.0f);
	} else {
		player->SetMoveSpeed(0.0f, 0.0f, 0.0f);
		player->SetTurnSpeed(0.0f, 0.0f, 0.0f);
	}
	TheCamera.RestoreWithJumpCut();
	CPad::bHasPlayerCheated = true;
	ShowLCSTeleportMessage("TELEPORTED NEAR DESTINATION");
	return true;
}

static void
TeleportLCSToCurrentSafehouse(void)
{
	CVector player = FindPlayerCoors();
	eLevelName level = CTheZones::GetLevelFromPosition(&player);
	CVector best;
	...
	for(int32 i = 0; i < NUMRADARBLIPS; i++) {
		...
		CVector position;
		if(!trace.m_bInUse || trace.m_eBlipDisplay == BLIP_DISPLAY_NEITHER ||
		   trace.m_eRadarSprite != RADAR_SPRITE_SAVE || !GetLCSBlipPosition(trace, position) ||
		   CTheZones::GetLevelFromPosition(&position) != level)
			continue;
	...
	if(!found) {
		ShowLCSTeleportMessage("NO SAFEHOUSE FOUND ON THIS ISLAND");
		return;
	}
	TeleportLCSToBlip(best);
}

static void
TeleportLCSToMainMission(void)
{
	if(CTheScripts::IsPlayerOnAMission()) {
		ShowLCSTeleportMessage("FINISH THE CURRENT MISSION FIRST");
		return;
	}
	CVector player = FindPlayerCoors();
	CVector best;
	...
	for(int32 i = 0; i < NUMRADARBLIPS; i++) {
		...
		CVector position;
		if(!trace.m_bInUse || trace.m_eBlipDisplay == BLIP_DISPLAY_NEITHER ||
		   !IsLCSMainMissionSprite(trace.m_eRadarSprite) || !GetLCSBlipPosition(trace, position))
			continue;
		float dx = position.x - player.x;
		float dy = position.y - player.y;
		float distance = dx * dx + dy * dy;
	...
	if(!found) {
		ShowLCSTeleportMessage("NO UNFINISHED MAIN MISSION FOUND");
		return;
	}
	TeleportLCSToBlip(best);
}
...
static void
ArmLCSFinalMissionBoatCheckpoint(void)
{
	if(CTheScripts::IsPlayerOnAMission()) {
		ShowLCSTeleportMessage("FINISH THE CURRENT MISSION FIRST");
		return;
	}
	/* Keep the request dormant until the SALH5 script itself starts.  This is
	 * safer than inferring finale availability from LastMissionPassedName: the
	 * player may be testing from an older save or may have completed another
	 * activity after SALH4. */
	FinalMissionMusic::ArmLCSFinaleBoatCheckpoint();
	CPad::bHasPlayerCheated = true;
	ShowLCSTeleportMessage("FINAL MISSION BOAT CHECKPOINT ARMED");
}

static bool
ActivateLCSCheatAlias(const char *cheat)
{
#define LCSCHEAT(name, action) if(!strcmp(cheat, name)){ action; return true; }
	LCSCHEAT("WEAPONS1", WeaponCheat1())
	LCSCHEAT("STREETWISE", WeaponCheat1())
	LCSCHEAT("WEAPONS2", WeaponCheat2())
	LCSCHEAT("BUSINESSASUSUAL", WeaponCheat2())
	LCSCHEAT("WEAPONS3", WeaponCheat3())
	LCSCHEAT("MADEMANARSENAL", WeaponCheat3())
	LCSCHEAT("MONEY", MoneyCheat())
	LCSCHEAT("FAMILYFORTUNE", MoneyCheat())
	LCSCHEAT("RECOVER", HealthCheat())
	LCSCHEAT("HEALTH", HealthCheat())
	LCSCHEAT("TOUGHASTHEYCOME", HealthCheat())
	LCSCHEAT("ARMOR", ArmourCheat())
	LCSCHEAT("ARMOUR", ArmourCheat())
	LCSCHEAT("SUITEDANDBOOTED", ArmourCheat())
	LCSCHEAT("WANTEDUP", WantedLevelUpCheat())
	LCSCHEAT("COMEANDGETME", WantedLevelUpCheat())
	LCSCHEAT("WANTEDOFF", WantedLevelDownCheat())
	LCSCHEAT("FORGETABOUTIT", WantedLevelDownCheat())
	LCSCHEAT("SUNNY", ExtraSunnyWeatherCheat())
	LCSCHEAT("HERECOMESTHESUN", ExtraSunnyWeatherCheat())
	LCSCHEAT("CLEAR", SunnyWeatherCheat())
	LCSCHEAT("NOTACLOUDINSIGHT", SunnyWeatherCheat())
	LCSCHEAT("CLOUDY", CloudyWeatherCheat())
	LCSCHEAT("CLOUDSOVERPORTLAND", CloudyWeatherCheat())
	LCSCHEAT("RAIN", RainyWeatherCheat())
	LCSCHEAT("RAININGONMYPARADE", RainyWeatherCheat())
	LCSCHEAT("FOG", FoggyWeatherCheat())
	LCSCHEAT("CANTFINDMYWAY", FoggyWeatherCheat())
	LCSCHEAT("RHINO", VehicleCheat(MI_RHINO))
	LCSCHEAT("TANKYOULIBERTY", VehicleCheat(MI_RHINO))
	LCSCHEAT("TRASHMASTER", VehicleCheat(MI_TRASH))
	LCSCHEAT("TAKEOUTTHETRASH", VehicleCheat(MI_TRASH))
	LCSCHEAT("FASTCLOCK", FastWeatherCheat())
	LCSCHEAT("TIMENEVERWAITS", FastWeatherCheat())
	LCSCHEAT("FASTGAME", FastTimeCheat())
	LCSCHEAT("LIFEINTHEFASTLANE", FastTimeCheat())
	LCSCHEAT("SLOWGAME", SlowTimeCheat())
	LCSCHEAT("SLOWANDSTEADY", SlowTimeCheat())
	LCSCHEAT("BLOWUP", BlowUpCarsCheat())
	LCSCHEAT("BOOMTOWN", BlowUpCarsCheat())
	LCSCHEAT("RIOT", MayhemCheat())
	LCSCHEAT("CITYGONECRAZY", MayhemCheat())
	LCSCHEAT("ATTACKME", EverybodyAttacksPlayerCheat())
	LCSCHEAT("EVERYBODYHATESME", EverybodyAttacksPlayerCheat())
	LCSCHEAT("PEDWEAPONS", WeaponsForAllCheat())
	LCSCHEAT("ARMEDANDDANGEROUS", WeaponsForAllCheat())
	LCSCHEAT("GREENTRAFFIC", TrafficLightsCheat())
	LCSCHEAT("ALLLIGHTSGREEN", TrafficLightsCheat())
	LCSCHEAT("AGGRESSIVEDRIVERS", MadCarsCheat())
	LCSCHEAT("ROADRAGE", MadCarsCheat())
	LCSCHEAT("BLACKCARS", BlackCarsCheat())
	LCSCHEAT("BACKINBLACK", BlackCarsCheat())
	LCSCHEAT("WHITECARS", WhiteCarsCheat())
	LCSCHEAT("WHITEWASH", WhiteCarsCheat())
	LCSCHEAT("CHROMECARS", ChromeCarsCheat())
	LCSCHEAT("BLINGBLING", ChromeCarsCheat())
	LCSCHEAT("DRIVEONWATER", BackToTheFuture())
	LCSCHEAT("FLOATMYBOAT", BackToTheFuture())
	LCSCHEAT("HANDLING", StrongGripCheat())
	LCSCHEAT("HANDLEWITHCARE", StrongGripCheat())
	LCSCHEAT("SUICIDE", SuicideCheat())
	LCSCHEAT("TONISLASTSTAND", SuicideCheat())
	LCSCHEAT("PEDSKIN", ChangePlayerCheat())
	LCSCHEAT("NEWFACEINTOWN", ChangePlayerCheat())
	LCSCHEAT("MEDIA", DoShowChaseStatCheat())
	LCSCHEAT("FIFTEENMINUTES", DoShowChaseStatCheat())
	LCSCHEAT("BIKETIRES", BikeTiresCheat())
	LCSCHEAT("WHEELDEAL", BikeTiresCheat())
	LCSCHEAT("CREDITS", CreditsCheat())
	LCSCHEAT("BIGHEADS", BigHeadsCheat())
	LCSCHEAT("FOLLOWME", MaleFollowersCheat())
	LCSCHEAT("BOYSCLUB", MaleFollowersCheat())
	LCSCHEAT("PASSENGER", PickUpChicksCheat())
	LCSCHEAT("HOPINPAL", PickUpChicksCheat())
	LCSCHEAT("UPSIDEDOWN", UpsideDownCheat(true))
	LCSCHEAT("TOPSYTURVY", UpsideDownCheat(true))
	LCSCHEAT("REVERSEUPSIDE", UpsideDownCheat(false))
	LCSCHEAT("RIGHTSIDEUP", UpsideDownCheat(false))
	/* Project/testing helpers intentionally keep their explicit names. */
	LCSCHEAT("TELEPORTH", TeleportLCSToCurrentSafehouse())
	LCSCHEAT("TELEPORTM", TeleportLCSToMainMission())
	LCSCHEAT("SKIP", ArmLCSFinalMissionBoatCheckpoint())
#undef LCSCHEAT
	return false;
}
#endif
...
CPad::AffectFrom3DS()
{
	...
	else if(!g3DSCheatKeyboardLatched && !FrontEndMenuManager.m_bMenuActive && FindPlayerPed() != nil){
		g3DSCheatKeyboardLatched = true;
		PCTempJoyState.Clear();
		Open3DSCheatKeyboard();
		return;
	}
	...
	PCTempJoyState.LeftShock = (held & KEY_TOUCH && g3DSTouchOverlayArmed &&
		g3DSTouchActiveZone == TOUCH_ZONE_L3) ? 255 : 0;
	PCTempJoyState.RightShock = (held & KEY_TOUCH && g3DSTouchOverlayArmed &&
		g3DSTouchActiveZone == TOUCH_ZONE_R3) ? 255 : 0;

	/* Circle Pad reaches about +/-156 at its cardinal limits.  Normalize before
	 * applying a radial deadzone so centre noise cannot move Toni while full
	 * travel still reaches the game's run and steering thresholds. */
	float lx = fmax(-1.0f, fmin(1.0f, (float)left.dx / 156.0f));
	float ly = fmax(-1.0f, fmin(1.0f, (float)left.dy / 156.0f));
	float rx = fmax(-1.0f, fmin(1.0f, (float)right.dx / 140.0f));
	float ry = fmax(-1.0f, fmin(1.0f, (float)right.dy / 140.0f));
	Apply3DSRadialDeadzone(lx, ly, 0.14f);
	Apply3DSRadialDeadzone(rx, ry, 0.18f);
	/* Match the production reVC New3DS response: preserve the radial deadzone,
	 * then compensate for the C-stick's much shorter physical travel. */
	rx = fmax(-1.0f, fmin(1.0f, rx * 2.3f));
	ry = fmax(-1.0f, fmin(1.0f, ry * 2.3f));
	PCTempJoyState.LeftStickX = (int32)(lx * 128.0f);
	PCTempJoyState.LeftStickY = (int32)(-ly * 128.0f);
	PCTempJoyState.RightStickX = (int32)(rx * 128.0f);
	PCTempJoyState.RightStickY = (int32)(-ry * 128.0f);
...
{
	FinalMissionMusic::CancelLCSFinaleBoatCheckpoint();
