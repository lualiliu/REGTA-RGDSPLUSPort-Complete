...
CreateEmbeddedCrackedGlassTexture(void)
{
	...
	if (lodepng_decode32(&rgba, &width, &height,
		wincrack_32_bin, wincrack_32_bin_size) != 0 ||
		width != 32 || height != 32) {
		free(rgba);
		return nil;
	}
	...
	RwUInt8 *pixels = RwImageGetPixels(image);
	const RwInt32 stride = RwImageGetStride(image);
	for (unsigned y = 0; y < height; y++)
		memcpy(pixels + y * stride, rgba + y * width * 4, width * 4);
	free(rgba);

	RwInt32 rasterWidth, rasterHeight, depth, format;
	...
	RwTextureSetName(texture, "wincrack_32");
