...
/* reLCS keeps most GPU textures and geometry in linear memory, but its script,
 * animation, frontend and save data all come from the application heap.  A
 * 60/56 MiB split left the latter smaller than the old 65 MiB streaming
 * budget and made late cutscenes fail on tiny allocations.  Linear resources
 * are reclaimable through the texture/streaming pressure path, so give the
 * application heap a permanent 12 MiB safety margin instead. */
