...
{
#ifdef _3DS
	/* EFX is neither required by the original PS2 audio path nor cheap on
	 * New3DS.  Keeping it disabled also removes dynamic EFX calls from the
	 * collision-sound path. */
	return false;
#else
...
static bool
OpenStreamedTrack(CStream *stream, uint32 track, char *filename)
{
	if (stream == nil)
		return false;

#ifdef _3DS
	/* Prefer host-converted mono IMA ADPCM on 3DS.  Its tiny integer decoder is
	 * substantially cheaper than continuous MP3 decoding while retaining a
	 * PSP-sized stream.  Keep MP3 and VB as transparent data-set fallbacks. */
	sprintf(filename, "%s.WAV", StreamedNameTable[track]);
	if (stream->Open(filename, IsThisTrackAt16KHz(track) ? 16000 : 32000))
		return true;
	sprintf(filename, "%s.MP3", StreamedNameTable[track]);
	if (stream->Open(filename, IsThisTrackAt16KHz(track) ? 16000 : 32000))
		return true;
	sprintf(filename, "%s.VB", StreamedNameTable[track]);
#else
	sprintf(filename, "%s.VB", StreamedNameTable[track]);
	if (stream->Open(filename, IsThisTrackAt16KHz(track) ? 16000 : 32000))
		return true;
	sprintf(filename, "%s.MP3", StreamedNameTable[track]);
#endif
	return stream->Open(filename, IsThisTrackAt16KHz(track) ? 16000 : 32000);
}
...
cSampleManager::Initialise(void)
{
	...
	} else
	{
		...
		{	
			...
			bool opened = OpenStreamedTrack(aStream[0], i, filename);
			if ( opened )
			{
...
cSampleManager::PreloadStreamedFile(uint32 nFile, uint8 nStream)
{
	...
	if ( nFile < TOTAL_STREAMED_SOUNDS )
	{
		...
		stream->Close();
		bool opened = OpenStreamedTrack(stream, nFile, filename);
...
bool8
cSampleManager::StartMissionMusicStream(const char *path, bool8 loop)
{
	const uint8 streamIndex = 1;
	CStream *stream = aStream[streamIndex];
	stream->Close();
	if (!stream->Open(path) || !stream->Setup()) {
		stream->Close();
		debug("Unable to open final-mission music stream %s\n", path);
		return FALSE;
	}
	stream->SetFullInitialQueue(TRUE);
	stream->SetLoopCount(loop ? 0 : 1);
	stream->SetSeamlessLoop(loop);
	stream->SetPan(63);
	stream->SetVolume(m_nMusicVolume);
	stream->Start();
	return TRUE;
}

void
cSampleManager::StopMissionMusicStream(void)
{
	aStream[1]->Close();
}

void
cSampleManager::PauseMissionMusicStream(bool8 pause)
{
	if (aStream[1]->IsOpened())
		aStream[1]->SetPause(pause != FALSE);
}

void
cSampleManager::SetMissionMusicStreamVolume(uint8 volume)
{
	if (aStream[1]->IsOpened())
		aStream[1]->SetVolume((uint32)volume * m_nMusicVolume / MAX_VOLUME);
}

bool8
cSampleManager::IsMissionMusicStreamPlaying(void)
{
	return aStream[1]->IsPlaying();
}

bool8
cSampleManager::HasMissionMusicStreamFinished(void)
{
	CStream *stream = aStream[1];
	if (!stream->IsOpened() || stream->IsPlaying())
		return FALSE;

	/* GetPosMS deliberately subtracts the stream's prequeued audio. Once the
	 * final queue has drained that still leaves the reported position about
	 * 750 ms short, so a position tolerance can miss a real EOF. Require both
	 * a stopped OpenAL queue and a decoder that has consumed the actual file. */
	return stream->HasDecodedToEnd();
}
