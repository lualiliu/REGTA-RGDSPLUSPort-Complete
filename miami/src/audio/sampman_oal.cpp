...
cSampleManager::PreloadStreamedFile(uint32 nFile, uint8 nStream)
{
	...
	if ( nFile < TOTAL_STREAMED_SOUNDS )
	{
		...
		CStream *stream = new CStream(filename, ALStreamSources[nStream], ALStreamBuffers[nStream],
			IsThisTrackAt16KHz(nFile) ? 16000 : 32000, nStream != 0, true, nStream);
		ASSERT(stream != NULL);
...
#ifdef _3DS
int8
cSampleManager::PreparePreloadedStreamedFile(uint8 nStream)
{
	ASSERT(nStream < MAX_STREAMS);
	CStream *stream = aStream[nStream];
	return stream ? stream->BeginPrepare() : -1;
}

int8
cSampleManager::GetPreloadedStreamedFileStatus(uint8 nStream)
{
	ASSERT(nStream < MAX_STREAMS);
	CStream *stream = aStream[nStream];
	return stream ? stream->GetPrepareStatus() : -1;
}
#endif
...
bool
cSampleManager::StartMissionMusicStream(const char *path, bool loop)
{
	/* Streams 1 and 2 are both used by Vice City's two mission-dialogue
	 * slots.  Keep finale music on its own stream so loading dialogue cannot
	 * delete the music (and music transitions cannot delete dialogue). */
	const uint8 streamIndex = 3;
	StopStreamedFile(streamIndex);
	CStream *stream = new CStream((char*)path, ALStreamSources[streamIndex], ALStreamBuffers[streamIndex]);
	if (stream == nil || !stream->Setup()) {
		delete stream;
		return false;
	}
	aStream[streamIndex] = stream;
	/* Keep the hardware-tested paired-source path, but begin finale music with
	 * all four 250 ms buffer pairs queued.  Its lightweight ADPCM stream may
	 * also recover two blocks after an unusually busy mission frame. */
	stream->SetFullInitialQueue(true);
	stream->SetMissionMusic(true);
...
void
cSampleManager::PauseMissionMusicStream(bool pause)
{
	PauseStream(pause, 3);
}
...
{
	CStream *stream = aStream[3];
	if (stream)
		/* This stream is mixed independently from the stock radio/cutscene
		 * stream.  Its own controller supplies cutscene ducking and the final
		 * fade, so applying m_nMusicFadeVolume here can mute the gameplay start. */
		stream->SetVolume((uint32)volume * m_nMusicVolume / MAX_VOLUME);
}
...
{
	CStream *stream = aStream[3];
	if (stream == nil || stream->IsPlaying())
		return false;
