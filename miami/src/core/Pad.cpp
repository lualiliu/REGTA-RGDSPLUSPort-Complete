...
void CPad::UpdateMouse()
{
	...
	if (down & KEY_TOUCH){
		...
		if(g3DSTouchOverlayArmed && g3DSTouchActiveZone == TOUCH_ZONE_CAMERA) {
			...
			if (dx || dy){
				// The PC mouse path is not consumed by the 3DS gameplay camera.
				// Preserve it for menus, and also feed the proven right-stick path
				// from AffectFrom3DS below.
				g3DSTouchCameraDX = dx;
				g3DSTouchCameraDY = dy;
	...
	// The old port treated a quick second tap as a PC left-click. The visible
	// three-zone panel replaces that ambiguous gesture on 3DS.
	PSGLOBAL(LMB) = 0;
	PCTempMouseControllerState.LMB = 0;
	OldMouseControllerState = NewMouseControllerState;
	NewMouseControllerState = PCTempMouseControllerState;
#endif
}
...
void CPad::AffectFrom3DS()
{
	...
	/* Circle Pad reports about +/-156 at its physical cardinal limits.  Using
	 * 200 here left a full tilt at only ~95/128 after the radial deadzone, so
	 * it could not reach the same run/steering thresholds as the D-pad. */
	float lx = fmax(-1.0f, fmin(1.0f, (float)thumbL.dx / 156.0f));
	float ly = fmax(-1.0f, fmin(1.0f, (float)thumbL.dy / 156.0f));
	...
	/* Keep the centre stable, but compensate for the C-stick's tiny travel. */
