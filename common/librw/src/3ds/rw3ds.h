...
inline void setMaterialColor(uint32 flags, const RGBA &color)
{
	...
	setMaterialColor(flags & Geometry::MODULATE ? color : white);
}
...
void setPlayerVehicleClump(Clump *clump);
void setPlayerVehicleReflection(Texture *texture, Frame *captureFrame);
void setPlayerVehicleReflectionStrength(float strength);
void setTrafficVehicleReflection(Texture *texture, Frame *captureFrame);
void setTransitionVehicleReflection(Clump *clump, Texture *texture,
...
struct C3DRaster
{
	...
	uint8 addressU;
	uint8 addressV;
	uint32 lastUsedSerial;
	bool shrinkProtected;
...
#ifdef RW_3DS
typedef bool (*TextureMemoryPressureCallback)(size_t requiredFreeBytes);

void *safeLinearAlloc(size_t size);
void TexTouch(C3DRaster *natras);
void TexSetShrinkProtected(C3DRaster *natras);
void TexSetMemoryPressureCallback(TextureMemoryPressureCallback callback);
#endif
...
void forgetRasterState(Raster *raster);
