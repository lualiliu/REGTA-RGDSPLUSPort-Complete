...
{
	CStream *stream = aStream[2];
	if (stream)
		/* Final-mission music has its own cutscene/finale fades.  Do not also
		 * multiply it by the stock radio fade, which is zero while the finale
		 * switches from its intro cutscene into gameplay. */
...
cSampleManager::HasMissionMusicStreamFinished(void)
{
	...
	if (stream == nil || stream->IsPlaying())
		return false;
	/* GetPosMS still subtracts the nominal prequeue after the final buffers have
	 * drained, which leaves the reported position short of the real EOF. */
