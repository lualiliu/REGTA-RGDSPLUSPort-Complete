...
{
	return weapon == WEAPONTYPE_AK47 || weapon == WEAPONTYPE_M16;
}
...
CPlayerPed::ProcessPlayerWeapon(CPad *padUsed)
{
	...
	if (!m_pFire) {
		...
		if (weapon == WEAPONTYPE_ROCKETLAUNCHER || weapon == WEAPONTYPE_SNIPERRIFLE
			...
			if (padUsed->TargetJustDown() || enterRifleFirstPerson) {
#ifdef _3DS
				...
				else if (weapon == WEAPONTYPE_SNIPERRIFLE)
...
#ifdef _3DS
			if (Is3DSRifleWeapon(FindPlayerPed()->GetWeapon()->m_eWeaponType)) {
#else
