...
static touchPosition g3DSTouchPrevious;
#endif
...
Open3DSCheatKeyboard(CPad *pad)
{
	SwkbdState keyboard;
	char cheat[20] = { 0 };
	swkbdInit(&keyboard, SWKBD_TYPE_QWERTY, 2, 19);
...
void CPad::UpdateMouse()
{
	...
	// Touch is owned by AffectFrom3DS, which exposes explicit L3/R3/camera
	// zones. Keep the legacy double-tap mouse path disabled to avoid one touch
	// producing both a virtual stick action and a menu click.
	PSGLOBAL(LMB) = 0;
...
void CPad::AffectFrom3DS()
{
	...
	if(down & KEY_TOUCH) {
		hidTouchRead(&touch);
		g3DSTouchPrevious = touch;
		g3DSTouchLastActivityMs = now;
		if(!g3DSTouchOverlayVisible) {
			// The first touch only reveals the controls. A release is required
			// before a zone can fire, preventing accidental horn/look input.
			g3DSTouchOverlayVisible = true;
			g3DSTouchOverlayArmed = false;
			g3DSTouchActiveZone = TOUCH_ZONE_NONE;
		} else if(g3DSTouchOverlayArmed) {
		...
		if(g3DSTouchOverlayArmed && g3DSTouchActiveZone == TOUCH_ZONE_CAMERA) {
			hidTouchRead(&touch);
			g3DSTouchCameraDX = touch.px - g3DSTouchPrevious.px;
			g3DSTouchCameraDY = touch.py - g3DSTouchPrevious.py;
			g3DSTouchPrevious = touch;
		}
