...
CStreaming::RequestModelStream(int32 ch)
{
	...
	// Keep the original adjacent read-ahead. Limiting 3DS to one item did not
	// reduce hardware stalls and only fragmented the streaming queue into more
	// SD reads; conversion-side memory pressure is handled in librw instead.
	const int32 maxFilesPerRead = 4;
	haveBigFile = 0;
	havePed = 0;
	totalSize = 0;
	for(i = 0; i < maxFilesPerRead; i++){
